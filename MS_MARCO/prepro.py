...
def process_file(filename, data_type, word_counter, char_counter, sampling):
    ...
    with open(filename, "r") as fh:
        ...
        organized, filtered_out = _organize(flat, 0, 10*q_num)
...
def _organize(flat, span_only, total):
    """
    ...
    for qid, passages, query, answers in tqdm(flat):
        ...
        if (len(passages) is not 10) or (answers is None) or (answers[0].find("No Answer Present.")>=0):
        ...
        for ind, passage in enumerate(passages):
            ...
            pos = ("NoAnSweR "+passage['passage_text']).find(ans)
            if pos >= 0:
                matching.add(ind)
                organized_buff.append((qid, passage, query, ans, (pos, pos+len(ans))))
                find_span = True 
            ...
            if not span_only:
                ...
                if passage.get('is_selected', False):
                    ...
                    organized_buff.append((qid, passage, query, ans, (0, 9)))
                else:
                    matching.add(ind)
                    organized_buff.append((qid, passage, query, ans, (0, 9)))
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        if q_id not in qid_example_dict.keys():
            qid_example_dict[q_id] = {"context_idxs":[], "ques_idxs":[], "context_char_idxs":[], "ques_char_idxs":[], "y1":[], "y2":[], "is_select":[], "count":0, "in_answer":[] }
    ...
    for qid in tqdm(qid_example_dict.keys()):
        ...
        if np.amax(np.argmax(y2_b, axis=-1))==0:
            continue
...
def prepro(config):
    ...
    test_examples, test_eval = process_file(
        config.test_file, "test", word_counter, char_counter, False)
